...
- 使用 Fabric.js 官方裁切控制，双击图片后直接在成品画布内拖动定位、拖四角缩放；无需在两个窗口间切换。
- 支持取消、恢复居中、完成裁切和最多40步撤销/重做，操作结束后自动保存。
- Sharp/libvips 后端读取原图并固定导出小红书竖版 `1080 × 1620` PNG；预览尺寸和手机屏幕大小不影响成图清晰度。
- 2×3、3×3、3×4、4×3、5×5 框架，以及多套本地样式模板。
- 风格模板默认应用整个项目；高级区仍可手动设置画布、边框、备注、标题颜色、圆角和格子间距。
...
- 原图渲染服务由 Docker 内部端口8767提供，网页自动调用，无需对局域网开放。
...
访问 `http://127.0.0.1:8765`，手机使用电脑的局域网IP。Vite会把草稿/素材请求转发到8766端口，并把高清导出请求转发到8767端口。
